...
def buildModel(dimensions, classes_num):
    ...
    model.add(Conv2D(32, (4, 4), input_shape=dimensions, padding="same"))  #1
    model.add(Activation("relu"))                                          #2  
    model.add(BatchNormalization(axis=chanDim))                            #3
...
def train(dimensions, X_train, Y_train, X_validation, Y_validation, epochs, batch_size, model_name):
    ...
    # SAVING THE MODEL
    pd.DataFrame.from_dict(history.history).to_csv(r'D:\\modelsTrained\\' + model_name[:-5] + 'csv', index=False)
    model.save(r'D:\\modelsTrained' + model_name)
    model.save(r'D:\\modelsTrained' + model_name[:-5] + 'h5')
