...
def load_dataset(train_path, valid_path, valid_csv, dimensions, preloaded=False):
    if not preloaded:
        ...
        np.save("./Datasets/IND/X_train.npy", X_train)
        np.save("./Datasets/IND/Y_train.npy", Y_train)
        np.save("./Datasets/IND/X_validation.npy", X_validation)
        np.save("./Datasets/IND/Y_validation.npy", Y_validation)
    else:
        print("\nLoading dataset from preloaded numpy arrays...")
        t1 = time()
        X_train = np.load("./Datasets/IND/X_train.npy")
        Y_train = np.load("./Datasets/IND/Y_train.npy")
        t1 = time() - t1
        print(f"Loaded training dataset in {t1:.4f} seconds")
        t2 = time()
        X_validation = np.load("./Datasets/IND/X_validation.npy")
        Y_validation = np.load("./Datasets/IND/Y_validation.npy")
        t2 = time() - t2
